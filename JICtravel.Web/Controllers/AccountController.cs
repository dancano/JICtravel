    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(AddUserViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                Data.Entities.SlaveEntity user = await _userHelper.AddUserAsync(model, path);
                if (user == null)
                {
        ...
        {
            SlaveEntity user = await _userHelper.GetUserAsync(User.Identity.Name);
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string token)
        {
            ...
            SlaveEntity user = await _userHelper.GetUserAsync(new Guid(userId));
            if (user == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> RecoverPassword(RecoverPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                SlaveEntity user = await _userHelper.GetUserAsync(model.Email);
                if (user == null)
                {
        ...
        {
            SlaveEntity user = await _userHelper.GetUserAsync(model.UserName);
            if (user != null)
            {
